...
Báo cáo đặt cơ sở dữ liệu SQL ở vị trí trung tâm trong hệ thống LMS CSDLNC. Các thành phần frontend và backend được sử dụng để làm rõ luồng nghiệp vụ, cách dữ liệu được tạo ra, được xử lý và được khai thác trong toàn bộ ứng dụng.
...
Bên cạnh full backup, hệ thống cần bật `binary log` trong MySQL để ghi nhận các thay đổi phát sinh sau thời điểm tạo file sao lưu. Đây chính là phần "backup thay đổi" giúp giảm nguy cơ mất dữ liệu nếu sự cố xảy ra giữa hai lần backup định kỳ.
...
Nội dung này tạo nền tảng để hệ thống tiếp tục phát triển lên các hướng nâng cao hơn như tự động hóa backup, sao lưu mã hóa, `point-in-time recovery` hoàn chỉnh và kết hợp với replication trong môi trường triển khai thực tế.
...
Phần ảnh minh họa giao diện thể hiện kết quả triển khai ứng dụng LMS trên nền tảng web và cho thấy dữ liệu SQL được khai thác trong các chức năng thực tế. Các màn hình tiêu biểu thể hiện luồng sử dụng của hệ thống từ xác thực tài khoản, xem khóa học, học video, làm bài kiểm tra, tham gia lớp học đến quản lý nội dung của giảng viên.
