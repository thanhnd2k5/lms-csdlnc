...
### 5.7. Hạn chế và minh chứng cần bổ sung

Phần thực nghiệm hiện đã có bốn hình `EXPLAIN` so sánh trước/sau chỉ mục cho hai truy vấn chính. Các hình này đủ để chứng minh sự khác biệt về kế hoạch thực thi và `query cost`. Nếu muốn phần báo cáo hoàn chỉnh hơn khi dàn vào Word, có thể bổ sung thêm hai loại minh chứng:

- ảnh kết quả truy vấn thực tế để cho thấy truy vấn trả về dữ liệu đúng;
- ảnh hoặc bảng ghi thời gian thực thi thực tế trên MySQL Workbench, DBeaver hoặc công cụ đang sử dụng.

Hai loại minh chứng này không bắt buộc nếu trọng tâm của chương là phân tích `EXPLAIN`, nhưng sẽ giúp chương 5 cân hơn với chương 6 và chương 7 vì có cả lệnh chạy, kết quả truy vấn, kế hoạch thực thi và nhận xét sau thực nghiệm.
