...
Kiến trúc này cho phép hệ thống vừa tận dụng replica để giảm tải đọc, vừa tạo cơ sở để kiểm thử các tình huống failover trong quá trình vận hành. Trong quá trình chạy, backend duy trì trạng thái topology gồm node ghi hiện tại và node chờ, nhờ đó vai trò của các node có thể được cập nhật lại sau mỗi lần failover hoặc rejoin.

`[Chèn Hình 7.1. Kiến trúc thực nghiệm replication, read/write split và automatic failover tại đây]`

Sơ đồ kiến trúc cho thấy mô hình replication được triển khai không chỉ ở tầng cơ sở dữ liệu mà còn có sự phối hợp với backend. Primary chịu trách nhiệm ghi dữ liệu, replica phục vụ đồng bộ và đọc dữ liệu, còn failover-manager theo dõi trạng thái để điều phối khi node chính gặp sự cố.
...
`[Chèn Hình 7.2. Hai node cơ sở dữ liệu được khởi động trong môi trường container tại đây]`

Minh chứng hai container cơ sở dữ liệu cùng hoạt động cho thấy môi trường thực nghiệm đã được dựng thành công. Đây là nền tảng để kiểm tra replication, vì cần có ít nhất một node primary và một node replica trước khi thiết lập đồng bộ dữ liệu.

`[Chèn Hình 7.3. Trạng thái replication giữa primary và replica tại đây]`

Trạng thái replication cho biết replica đã kết nối và bám theo primary. Các thông tin trạng thái trong hình giúp xác nhận luồng sao chép dữ liệu đang hoạt động, từ đó có cơ sở kiểm thử việc ghi dữ liệu ở primary và đọc lại ở replica.
...
`[Chèn Hình 7.4. Dữ liệu được ghi từ ứng dụng và đồng bộ sang replica tại đây]`

Hình này chứng minh dữ liệu được ghi từ ứng dụng vào primary có thể xuất hiện ở replica sau quá trình đồng bộ. Đây là minh chứng trực tiếp cho hiệu quả của replication, vì dữ liệu không chỉ tồn tại trên node ghi mà còn được sao chép sang node dự phòng.

`[Chèn Hình 7.5. Log backend khi tách truy vấn đọc và ghi tại đây]`

Log backend thể hiện cơ chế tách truy vấn đọc và ghi ở tầng ứng dụng. Các truy vấn ghi được định tuyến tới primary, trong khi truy vấn đọc được chuyển sang replica, giúp giảm tải cho node chính và tận dụng được bản sao dữ liệu.
...
Các tham số thực nghiệm được cấu hình trong backend như sau:

| Tham số | Giá trị mặc định | Ý nghĩa |
| --- | --- | --- |
| `DB_HEALTHCHECK_INTERVAL_MS` | `5000ms` | Chu kỳ `failover-manager` kiểm tra trạng thái node ghi hiện tại. |
...
Sau thời điểm đó, backend vẫn tiếp tục thực hiện được các thao tác `INSERT`, trong khi các truy vấn đọc vẫn hoạt động bình thường. Kết quả này cho thấy cơ chế automatic failover đã vận hành đúng theo thiết kế trong môi trường kiểm thử. Đồng thời, trạng thái topology của hệ thống cũng được cập nhật để node primary mới trở thành writer hiện tại cho các thao tác tiếp theo.

Về mặt triển khai, quy trình promote replica được thực hiện bằng script kết hợp với logic điều phối ở tầng backend. Trong nội dung chính của chương, báo cáo tập trung vào cơ chế hoạt động, quy trình xử lý và kết quả kiểm thử, còn các trích đoạn cấu hình và script tiêu biểu được đưa xuống phụ lục để tránh làm rối mạch trình bày.

`[Chèn Hình 7.6. Automatic failover khi node primary gặp sự cố tại đây]`

Minh chứng failover cho thấy khi primary gặp sự cố, hệ thống có thể promote replica thành node ghi mới. Điều này giúp hệ thống duy trì khả năng phục vụ thay vì dừng hoàn toàn khi node cơ sở dữ liệu chính bị lỗi.

`[Chèn Hình 7.7. Hệ thống tiếp tục ghi dữ liệu sau khi failover hoàn tất tại đây]`

Sau khi failover hoàn tất, hệ thống vẫn tiếp tục ghi được dữ liệu, chứng tỏ backend đã cập nhật node ghi hiện tại. Kết quả này cho thấy mô hình không chỉ sao chép dữ liệu, mà còn có khả năng chuyển vai trò để duy trì hoạt động.
...
Nhờ vậy, sau failover hệ thống vẫn có thể đưa node cũ quay trở lại cụm mà không gây xung đột vai trò ghi. Sau khi rejoin hoàn tất, backend cập nhật lại node này vào vai trò standby, qua đó khôi phục trạng thái hai node để sẵn sàng cho các vòng failover tiếp theo. Đây chưa phải là `automatic failback`, nhưng là một bước hoàn thiện quan trọng, giúp mô hình chặt chẽ hơn và dễ bảo vệ hơn về mặt kỹ thuật.

Tương tự như phần promote replica, quy trình `manual rejoin` cũng được hiện thực bằng script hỗ trợ cấu hình lại node cũ để bám theo primary mới. Việc tách các đoạn script tiêu biểu xuống phụ lục giúp nội dung chính của chương gọn hơn và giữ được trọng tâm phân tích kỹ thuật.

`[Chèn Hình 7.8. Node primary cũ được đưa trở lại hệ thống ở vai trò replica tại đây]`

Việc đưa primary cũ quay lại với vai trò replica giúp hệ thống khôi phục cấu trúc primary - replica sau sự cố. Dù bước rejoin còn mang tính thủ công, minh chứng này cho thấy nhóm đã kiểm tra được vòng đời xử lý sự cố gồm phát hiện lỗi, failover và đưa node cũ trở lại hệ thống.
